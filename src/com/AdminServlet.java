              							...
              							throws ServletException,IOException
    {
    	...
    	{//搜索商品时
    		...
    		{//按输入的文字搜索时
	    		...
	    		//得到搜索信息的sql和信息条数的sql
	    		sql = "select Bimgurl,Bname,Bintro,Bclass,Bprice,Blook,Bid,"+
	    						"Bdate from booksinfo where Bname like '%"+tsearch+"%'";
	    		sqlpage = "select count(*) from booksinfo "+
	    									"where Bname like '%"+tsearch+"%'";    			
    		}
    		else
    		{//按类别搜索时
    			cname = new String(cname.trim().getBytes("ISO-8859-1"),"gb2312");
	    		//得到搜索类别信息的sql和信息条数的sql
	    		sql = "select Bimgurl,Bname,Bintro,Bclass,Bprice,Blook,Bid,"+
	    						"Bdate from booksinfo where Bclass='"+cname.trim()+"'";
	    		sqlpage = "select count(*) from booksinfo "+
	    									"where Bclass='"+cname.trim()+"'";    			
    		}
    		adBean.setSql(sql);
    		//设置总页数
    	...
    	{//管理商品时
    		...
    		String sql = "select Bid,Bname,Bprice,Bamount,Bclass,Bdate,Bimgurl,"+
    						"Bintro,Bbrief from booksinfo where Bid='"+Integer.parseInt(gid)+"'";
    		//得到该商品的信息
    	...
    	{//增加商品
    		...
    		int gid = DBcart.getID("booksinfo","Bid");
    		String sql = "";
    		if(!gclass.equals("")&&!gurl.equals(""))
    		{//均输入商品类别和图片URL
	    		//组合成sql语句
	    		sql = "insert into booksinfo(Bid,Bname,Bprice,"+
	    						"Bamount,Bdate,Bclass,Bimgurl,Bintro,Bbrief)"+
    		...
    		{//输入商品类别,但图片URL没有输入
	    		sql = "insert into booksinfo(Bid,Bname,Bprice,"+
	    						"bamount,Bdate,Bclass,Bintro,Bbrief)"+
	    						" values("+gid+",'"+gname+"',"+Double.parseDouble(gprice)+
	    						","+Integer.parseInt(gamount)+",now(),'"+gclass+
	    						"','"+gintro+"','"+gbrief+"')";   			
    		}
    		...
    		{//输入图片URL,但没有商品类别
	    		sql = "insert into booksinfo(Bid,Bname,Bprice,"+
	    						"Bamount,Bdate,Bimgurl,Bintro,Bbrief)"+
	    						" values("+gid+",'"+gname+"',"+Double.parseDouble(gprice)+
    	...
    	{//修改商品信息
    		...
    		//得到要更新的Sql语句
    		String sql = "update booksinfo set Bname=\""+gname+"\","+
    						"Bprice="+price+",Bamount="+amount+",Bclass='"+
    						gclass+"',Bdate='"+gdate+"',Bimgurl='"+gurl+"',"+
    						"Bintro='"+gintro+"',Bbrief='"+gbrief+"' where Bid='"+id+"'";
    		//将sql转码,并执行更新
    		//sql = new String(sql.getBytes(),"ISO-8859-1");
    		int i = DBcart.updateTable(sql);
    		String msg = "";
    	...
    	{//删除商品
    		...
    		int id = Integer.parseInt(gid);
    		//当删除商品时,只将该商品数量置为0
    		String sql = "update booksinfo set Bamount=0 where Bid='"+id+"'";
    		int i = DBcart.updateTable(sql);
    		String msg = "";
    		if(i==1)
    		{
    	...
    	{//点击查看/管理时
    		...
    		//得到订单中货物信息
    		String ogsql = "select Bname,OBamount,Obtotalprice from booksinfo,"+
    							"orderbooks where booksinfo.Bid=orderbooks.Bid"+
    							" and Oid='"+id+"'";
    	...
    	{//某订单确认时
    		...
    		String temp = "select Bid,OBamount from orderbooks where Oid='"+id+"'";
    		Vector<String[]> vtemp = DBcart.getInfoArr(temp);
    		String[] str = new String[vtemp.size()+1];
    		for(int i=0;i<vtemp.size();i++)
    		{
    			String[] arr = vtemp.get(i);
    			str[i] = "update booksinfo set Bamount=Bamount-"+
    							Integer.parseInt(arr[1])+" where Bid="+arr[0];
    		}
    		str[vtemp.size()] = sql;
    	...
    	{//删除某订单时
    		...
    		str[0] = "delete from orderbooks where Oid='"+id+"'";
...
//    		System.out.println(flag);
    	...
    	{//删除用户
    		...
    		pageForward(msg,"/usermanage.jsp",request,response);
    	}
    	else if(action.equals("message"))
    	{
    		...
    		String msql = "select Mid,user_name,content,release_time from messageboard"+
    							" where Mid='"+id+"'";
    		Vector<String[]> messageinfo = DBcart.getInfoArr(msql);
			request.setAttribute("messageinfo",messageinfo);
			String url = "/repmessage.jsp";
			ServletContext sc = getServletContext(); 
			RequestDispatcher rd = sc.getRequestDispatcher(url); 
			rd.forward(request,response);
    	}
    	...
    	{//删除留言
    		...
    		int id = Integer.parseInt(mid);
    		//当删除商品时,只将该商品数量置为0
    		String sql = "delete from messageboard where Mid='"+id+"'";
    		int i = DBcart.updateTable(sql);
    		String msg = "";
    		if(i==1)
    		{
    	...
    	else if(action.equals("reply"))
    	{
    		...
    		//得到要更新的Sql语句
    		String sql = "update messageboard set reply_content='"+replycontent+"' where Mid='"+id+"'";
