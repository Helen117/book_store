...
//    		System.out.println("+11111111+");
    		...
    		String sql = "select Uid from userinfo where Uname='"+uid+"' and Upwd='"+pwd+"'";
//    		System.out.println(sql);
    	...
    	{//用户搜索商品时
    		...
    		if(cname==null)
    		{
	    		...
	    		sql = "select Bimgurl,Bname,Bintro,Bclass,Bprice,Blook,Bid "+
	    						"from booksinfo where Bname like '%"+tsearch+"%'";
    		...
    		else
    		{
    			...
    			//得到搜索某类商品的sql和条数的sql
	    		sql = "select Bimgurl,Bname,Bintro,Bclass,Bprice,Blook,Bid "+
	    						"from booksinfo where Bclass='"+cname.trim()+"'";
	    		//得到该类的总页数
	    		sqlpage = "select count(*) from booksinfo where Bclass='"+cname.trim()+"'";
    		}
    		mycart.setSql(sql);
    		//设置总页数
    	...
    	{//用户点击购买时
			...
			{
				url = "/goodsdetail.jsp";
				sql = "select Bimgurl,Bname,Bintro,Bclass,Bprice,"+
    						"Blook,Bid,Bbrief from booksinfo where Bid='"+gid+"'";
    			page = 1;
			}
    		//返回后,得到页面内容
    	...
    	{//用户修改购物车中商品数量时
    		...
    		//得到库存数量
    		String sql = "select Bamount from booksinfo where Bid='"+id+"'";
    	...
    	{//当点击结算时，判断各商品数量是否够
    		...
    		for(String str:gid)
    		{
    			...
    			{
    				//得到该商品的名字
    				String temp = "select Bname from booksinfo where Bid='"+id+"'";
    	...
    	{//当用户确认订单动作时
	    	...
	    	else
	    	{
	    		...
	    		int ogid = DBcart.getID("orderbooks","OBid");    		
	    		...
	    		for(int i=0;i<vgoods.size();i++)
	    		{
	    			...
	    			String temp = "insert into orderbooks(OBid,Oid,Uid,Bid,OBamount,"+
	    						"OBtotalprice) values("+ogid+","+oid+","+uid+","+gid+
	    						","+gamount+","+totalprice+")";
	    			sqlarr[i] = temp;
	    			ogid++;
	    		}
    	...
    	{//用户请求得到某商品的详细信息时
	    	//得到商品ID
    		String gid = request.getParameter("gid").trim();
    		String sql = "select Bimgurl,Bname,Bintro,Bclass,Bprice,"+
    						"Blook,Bid,Bbrief from booksinfo where Bid='"+gid+"'";
    		//更新表中的浏览量
    		String updatesql = "update booksinfo set Blook=Blook+1 where Bid='"+gid+"'";
    		...
    		int mid = DBcart.getID("messageboard","Mid");
    		String sql = "insert into messageboard(Mid,user_name,Uemail,"+
	    						"tele,company,release_time,content)"+
	    						" values("+mid+",'"+username+"','"+email+"','"+tele+
	    						"','"+company+"',now(),'"+content+"')"; 
    		//System.out.println(sql);
    		int i = DBcart.updateTable(sql);
    		String msg = "";
